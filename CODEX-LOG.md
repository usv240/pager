...
## Task 19 — Two complete, manifest-owned missions
- **What Codex did:** Promoted the Python invoice-queue retry mission to the default complete path, added three authored Python repair candidates, artifact-owned telemetry and stakeholder context, and a Pyodide verification drawer.
- **Where it accelerated us:** Reused the same manifest, recommendation, decision, and deterministic-verification contracts instead of creating a separate Python-only learning experience.
- **Key decisions we made (human):** Offer Python and TypeScript as the two complete demo languages; do not claim Java or C++ support until isolated compiler sandboxes exist; keep all base incident narrative with the incident artifact.
- **GPT-5.6 usage:** The optional AI Pair can answer scoped learner questions. Candidate code, stakeholder context, and pass/fail results are committed artifacts and deterministic runners remain the authority.

## Task 20 — Candidate-proof parity
- **What Codex did:** Added repository-local isolated candidate verification for checkout repairs and a Python verifier that executes each full-file Python candidate against the real `unittest` suite.
...
## Task 22 - Workbench hardening fixes
- **What Codex did:** Installed WebContainer dependencies once per verification session instead of every run, with distinct install-versus-test failure states and progress messaging; added a side-by-side current-versus-proposed code diff before a learner can apply or reject an AI-pair repair, replacing the rationale-only view; surfaced assertion evidence in failed test results instead of a bare “Failed”; fixed timer contrast states and removed root-cause spoilers from pre-decision copy; corrected incident availability labeling so only the fully spec-reviewed, execution-verified checkout mission is marked complete, with newer unverified labs marked experimental.
- **Where it accelerated us:** Diagnosed and fixed the WebContainer reinstall bottleneck, wired the diff view into the existing review flow, and completed the copy and labeling sweep across multiple manifest and component files.
- **Key decisions we made (human):** Verification by execution remains the only authority—the diff view exists so learners judge code, not prose; pre-decision copy must never reveal the root cause as narrative content expands; only missions that have completed the full spec-to-verification pipeline are labeled complete.
- **GPT-5.6 usage:** None required for this task—pure engine and UI hardening, with no runtime model calls involved. Codex Session ID (workbench-fixes thread): `019f8166-5e3a-7f60-8c4f-7e026707c843`.
