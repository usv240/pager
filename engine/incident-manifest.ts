...
export function parseManifest(value: unknown): IncidentManifest {
  ...
  if (!manifest.title || !manifest.service || !manifest.severity || !manifest.alert || !manifest.serviceDirectory || !manifest.activeFile || !manifest.execution) throw new Error("Incident manifest is missing required fields.");
