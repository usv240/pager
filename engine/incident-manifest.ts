...
export function parseManifest(value: unknown): IncidentManifest {
  ...
  if (!manifest.title || !manifest.difficulty || !manifest.service || !manifest.severity || !manifest.alert || !manifest.timeLimitSeconds || !manifest.availability || !manifest.briefing || !manifest.telemetry || !manifest.stakeholderMessages || !manifest.serviceDirectory || !manifest.activeFile || !manifest.execution) throw new Error("Incident manifest is missing required fields.");
  ...
  if (!Array.isArray(manifest.stakeholderMessages) || manifest.stakeholderMessages.length === 0 || manifest.stakeholderMessages.some((message) => !message.id || !["pm", "senior", "sre", "support", "finance", "ai-pair"].includes(message.role) || !message.author || !message.body || !message.timestamp)) throw new Error("Incident manifest has invalid stakeholder messages.");
