...
import { pythonSettlementReplayCandidates } from "./python-settlement-replay";
import { typescriptWebhookReplayCandidates } from "./typescript-webhook-replay";

export const allAuthoredCandidates = [...checkout2pmCandidates, ...pythonInvoiceQueueCandidates, ...pythonInventoryReservationCandidates, ...pythonSettlementReplayCandidates, ...typescriptWebhookReplayCandidates];
...
export function candidatesForFiles(paths: string[]) {
  ...
  if (paths.includes("src/replay_service.py")) return pythonSettlementReplayCandidates;
