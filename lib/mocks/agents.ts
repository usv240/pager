...
export const mockFixes: FixCandidate[] = [
  { id: "increase-timeout", title: "Increase the gateway timeout", faultTag: "symptom-not-cause", rationale: "The alert presents as a gateway error, so a longer timeout looks safe—but it does not make the read/create sequence atomic.", targetFile: "src/services/checkout-service.ts", patch: "await gateway.charge(order.total, order.paymentToken, { timeout: 10_000 });" },
  { id: "idempotency-key", title: "Use an idempotency key for the charge", faultTag: "partial-fix", rationale: "This reduces duplicate gateway charges, but without a transaction the payment record can still diverge from the charge result.", targetFile: "src/services/checkout-service.ts", patch: "const charge = await gateway.charge(order.total, order.paymentToken, { idempotencyKey: order.id });" },
  { id: "atomic-payment", title: "Claim the payment atomically", faultTag: "verified", rationale: "Move the order claim into a unique, atomic create. Only the request that wins may invoke the gateway; contenders return the existing record.", targetFile: "src/services/checkout-service.ts", patch: "const payment = await payments.claimOrFind(order.id);\nif (payment.chargeId) return payment;\nconst charge = await gateway.charge(order.total, order.paymentToken);\nreturn payments.attachCharge(payment.id, charge.id);" },
];
